<template>
    <div class="p-4 sm:ml-64 mt-14 bg-gray-100">
        <nav class="flex mb-5 px-5 py-1 text-gray-700 shadow border border-gray-200 rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700"
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span
                            class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Administrators</span>
                    </div>
                </li>
            </ol>
            ...
            <div class="ml-auto mt-3 flex justify-end">
                <button type="button"
                    class=" w-full inline-flex items-center justify-center text-gray-900 hover:text-white border border-gray-800 hover:bg-gray-900 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-800"
        ...
        <div class="grid grid-cols-1 mb-5 shadow sm:rounded-lg border border-gray-200 bg-white">
            <div class="relative overflow-x-auto  p-5">
                ...
                <table id="users_table" class="w-full text-left text-gray-500 dark:text-gray-400 display">
                    ...
                    <tbody>
                        <tr v-for="(row, index) in users" :key="row.id"
                            ...
                            <td class="px-6 py-4 ">
                                {{ row.user_status ? 'Activated' : 'Deactivated'}}
                            </td>
                            <td class="px-6 py-4 ">
                                {{ formatDate(row.created_at) ?? '-' }}
                            </td>
                            <td class="px-6 py-4 ">
                                <button id="dropdownLeftButton" v-bind:id="'account-settings' + row.id"
                                    data-dropdown-toggle="dropdownLeft" data-dropdown-placement="left"
                                    class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
                                    type="button"
                                    @click="accountSetting('account-settings'+row.id, row.id, row.user_status)"> <svg
                ...
                <button type="button" data-modal-target="addUserModal" data-modal-toggle="addUserModal"
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="flex p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400"
                            ...
                            <div>
                                <span class="font-medium">Important:</span> Enter valid emall address for email
                                notification.
                            </div>
                        </div>
                        <ValidationObserver v-slot="{ handleSubmit }">
                            <form @submit.prevent="handleSubmit(createNewUser)">
                                <div class="mb-6">
                                    <label for="email"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email
                                        address</label>
                                    <ValidationProvider rules="required" v-slot="{ errors }">
                                        <input @blur="checkEmail()" type="email" v-model="create_user_form.email"
                                    ...
                                    <ValidationProvider rules="required|min:8" v-slot="{ errors }">
                                        <div class="relative">
                                            <input type="password" name="password" id="password"
                                                v-model="create_user_form.password"
                                ...
                                <div class="mb-6">
                                    ...
                                    <ValidationProvider rules="required" v-slot="{ errors }">
                                        <input type="text" v-model="create_user_form.name"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                ...
                                <div class="mb-6">
                                    ...
                                    <ValidationProvider rules="required" v-slot="{ errors }">
                                        <select v-model="create_user_form.role"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        ...
        <div id="dropdownLeft"
            ...
            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownLeftButton">
                <li>
                    <a v-bind:href="userSettings"
                        class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Account
                        Settings</a>
                </li>
                <li>
                    <a @click="deleteAccount(userID)"
                        class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Delete
                        Account</a>
                </li>
                <li>
                    <a @click="setStatus(userID, userStatus)"
                        class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ statusAction }}</a>
                </li>
            </ul>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            checkEmail: function () {
                const email = this.create_user_form.email
                ...
                if (this.emailExist == '') {
                    Swal.fire({
                        ...
                        if (result.isConfirmed == true) {
                            ...
                            axios.post(APP_URL + '/admin/create_user', this.create_user_form)
                                .then(response => {
                            ...
                            let timerInterval
                            Swal.fire({
                                title: 'Create New Account Successfull.',
                                html: 'Reloading page.',
                                icon: 'success',
                                timer: 3000,
            ...
            accountSetting: function (accountSetting, id, status) {
                console.log(accountSetting + ' ' + id + ' ' + status)

                const $targetEl = document.getElementById('dropdownLeft')
                const $triggerEl = document.getElementById(accountSetting)
                const options = {
                    placement: 'left',
                    triggerType: 'click',
                    offsetSkidding: 0,
                ...
                this.userSettings = APP_URL + '/admin/user/settings/' + id
        ...
        mounted() {
            ...
            // data
            this.users = JSON.parse(JSON.stringify(this.usersData))
        }
    }
</script>